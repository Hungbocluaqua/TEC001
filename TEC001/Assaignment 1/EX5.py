#Quy đổi từ talents, pounds, lots sang gram
try:
    talents = input("Enter numbers of talents: ")
    pounds = input("Enter numbers of pounds: ")
    lots = input("Enter numbers of lots: ")
    if talents:
        print("grams from talents: ", f"{float(talents)*8.512: .2f}")
    else:
        print("You didn't enter any talents.")
    if pounds:
        print("grams from pounds: ", f"{float(pounds)*0.4256: .2f}")
    else:
        print("You didn't enter any pounds.")
    if lots:
        print("grams from lots : ", f"{float(lots)*0.0133: .2f}")
    else:
